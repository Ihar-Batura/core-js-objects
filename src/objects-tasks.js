...
function makeWord(lettersObject) {
  const arrKeys = Object.keys(lettersObject);
  const arrValues = Object.values(lettersObject).flat(10);
  const result = [];
  for (let i = 0; i < arrValues.length; i += 1) {
    for (let j = 0; j < arrKeys.length; j += 1) {
      if (lettersObject[arrKeys[j]].includes(i)) {
        result.push(arrKeys[j]);
      }
    }
  }
  return result.join('');
...
function sellTickets(queue) {
  const cashbox = queue.slice(0, -1);
  const customerPay = queue.slice(-1);
  const amount = cashbox.reduce((acc, number) => acc + number, 0) + 25;
  return amount - customerPay >= 0;
}
...
function Rectangle(width, height) {
  ...
  Rectangle.prototype.getArea = function showArea() {
